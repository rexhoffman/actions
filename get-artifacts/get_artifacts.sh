...
function usage() {
  ...
  echoerr -z unzip and delete the original downloaded artifacts?
...
if [ -n "$TOKEN" ]; then
  curl_attri+=('-H' "Authorization: token ${TOKEN}")  #"authorization: Bearer ${GITHUB_TOKEN}"
fi
echoerr Curl Parameters: "${curl_attri[@]}"


# Conditionally retrieves the artifacts from a single jobs, the input parameter which must correspond to a numeric workflow_id in github actions
# and stores the desired artifacts, contained in a list called "ARTIFACTS", in a subdirectory of TARGET_DIR, equal to the workflow_id.  Artifacts are always zips.
# The zip file is downloaded, unziped in to the job's sub directory in the TARGET_DIR, and then deleted.   Should the job dir already exist, no downloads are attempted,
# as the assumption would be this function has already populated the artifacts in to the job dir.
#
# INPUT:
#  Parameter 1:  A number workflow_id
#
# ENVIRONMENT:
...
getArtifactsFromJob() {
  jobId=$1;
  if [ ! -d "${TARGET_DIR}/${jobId}" ]; then
    mkdir -p "${TARGET_DIR}/${jobId}"
    ...
    for artifact_name in "${ARTIFACTS[@]}"; do
      ...
      if [ -n "$download_url_str" ] && [ "$download_url_str" != "null" ]; then
        ...
        curl -L "${curl_attri[@]}" "$download_url" -o "${TARGET_DIR}/${jobId}/${artifact_name}.zip" 2>/dev/null
        if [ "$UNZIP" == "true" ]; then
          unzip -q "${TARGET_DIR}/${jobId}/${artifact_name}.zip" -d "${TARGET_DIR}/${jobId}"
          rm "${TARGET_DIR}/${jobId}/${artifact_name}.zip"
        fi
      fi
    done
    echoerr fetched:
